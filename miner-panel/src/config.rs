...
use crate::presets::{
    CpuPreset, GpuPreset, cpu_idx_for_supervene, gpu_idx_for_profile, gpu_idx_for_slug,
    min_work_groups_for_gpu, resolve_panel_tuning, tuning_for_profile,
};
...
pub fn apply_loaded_ini(
    ...
    if let Some(sv) = loaded.supervene {
        if let Some(i) = cpu_idx_for_supervene(cpus, sv) {
            *cpu_idx = i;
        }
    }
...
/// `dynamic_supervene` rebalances CPU threads against a GPU in the same process
/// by watching the ratio of GPU nonces to CPU nonces. It therefore needs a GPU
/// to be true about anything.
///
/// Written as `false` for the diamond miner, which is CPU-only by design: there
/// the GPU nonce count is permanently zero, so the balancer sees a ratio of 0.0
/// forever and ratchets its counter to `supervene_max` and stays there. Nothing
/// would move, because that counter drives only the reported thread count and
/// the watt estimate, not the threads. All it could do is make an honest number
/// wander. See `app::mining_runtime::maybe_adjust_supervene`.
...
",
        ...
        sv = supervene,
        // `spawn_supervene` ignores the flag unless `supervene_max > 0`, so a
        // true here with a zero cap would be a claim the code cannot honour.
        dynamic_supervene = dynamic && supervene > 0,
...
/// CPU threads this panel really asks a worker to run, from the one number the
/// operator picked.
///
/// The panel has ONE CPU picker and three jobs behind it, and they do not want
/// the same number:
///
/// - HACD: the CPU is the entire miner, so the pick is written as chosen. Only
///   "GPU only" needs a substitute, and the substitute is this machine, not the
///   1 it used to become.
/// - HAC with no GPU: same situation, so the same answer, minus the substitute
///   (0 there means the operator really did choose to mine nothing).
/// - HAC with a GPU: the pick becomes CPU-ASSIST threads standing next to a card
///   worth several hundred of them, and the thread that feeds that card has to
///   compete with every one of them for a core. The ladder now reaches every
///   thread on the CPU, so without this cap an operator who set "all cores" for
///   diamonds and then switched to HAC would hand a GPU rig 30 assist threads
///   and starve the feed. Capped to `cpu_threads::cap_cpu_assist`.
///
/// Zero survives every path where zero is a real choice.
pub fn effective_supervene(chosen: u32, gpu_slug: &str, hacd: bool) -> u32 {
    if hacd {
        return match chosen {
            0 => app::cpu_threads::hacd_threads(),
            n => n,
        };
    }
    if gpu_slug == "none" {
    ...
    app::cpu_threads::cap_cpu_assist(chosen)
}
...
pub fn write_poworker_config(path: &Path, s: &PanelSettings) -> std::io::Result<()> {
    ...
    let supervene = effective_supervene(s.cpu.supervene, s.gpu.slug, false);
    let cpu_assist = !cpu_only && supervene > 0;
...
",
        ...
        efficiency = efficiency_section(
            ...
            s.max_temp_c,
            supervene,
            // A GPU rig has something to rebalance against; a CPU-only HAC rig
            // does not, for the same reason HACD does not.
            !cpu_only,
        ),
...
pub fn write_diaworker_config(path: &Path, s: &PanelSettings) -> std::io::Result<()> {
    ...
    let supervene = effective_supervene(s.cpu.supervene, "none", true);
    let body = format!(
...
",
        connect = s.connect,
        sv = supervene,
        efficiency = efficiency_section(s, 1, 0.0, 0, supervene, false),
    );
    crate::hacash_config::atomic_write_private(path, &body)
}
...
mod write_tuning_tests {
    ...
    fn hacd_config_is_strictly_cpu_only() {
        ...
        let raw = std::fs::read_to_string(&path).unwrap();
        let _ = std::fs::remove_file(path);
        // `panel_with_wg` selects preset 0, "GPU only". For a CPU-only worker
        // that used to be written as 1 thread; it is now this machine's count.
        assert!(
            raw.contains(&format!("supervene = {}", app::cpu_threads::hacd_threads())),
            "{raw}"
        );
        assert!(raw.contains("dynamic_supervene = false"), "{raw}");
    ...
    /// The panel has one CPU picker and it now reaches every thread on the CPU.
    /// For diamonds that is the point. For a GPU rig the same pick would become
    /// CPU-assist threads competing with the thread that feeds the card, and the
    /// card is worth several hundred of them. So the pick is capped on its way
    /// into `poworker.config.ini` and written whole into `diaworker.config.ini`.
    #[test]
    fn a_gpu_rig_never_inherits_the_hacd_thread_count() {
        let logical = app::cpu_threads::logical_cpus();
        let all_cores = app::cpu_threads::hacd_threads_for(logical);
        let safe_assist = app::cpu_threads::cpu_assist_threads_for(logical);

        assert_eq!(
            effective_supervene(all_cores, "rx9070xt", false),
            safe_assist
        );
        assert_eq!(effective_supervene(all_cores, "none", true), all_cores);
        // A CPU-only HAC rig has no feed thread to protect, so no cap.
        assert_eq!(effective_supervene(all_cores, "none", false), all_cores);
        // "No CPU mining" stays a choice on every path where it is one.
        assert_eq!(effective_supervene(0, "rx9070xt", false), 0);
        assert_eq!(effective_supervene(0, "none", false), 0);
        // ...and is not one for a CPU-only worker, which would mine nothing.
        assert_eq!(
            effective_supervene(0, "none", true),
            app::cpu_threads::hacd_threads()
        );
        // A modest pick is never raised.
        assert_eq!(effective_supervene(1, "rx9070xt", false), 1);
    }

    /// The cap has to survive into the file, including into `supervene_max`,
    /// or the worker's dynamic rebalancer would climb straight back to the
    /// uncapped number the panel refused to write on the first line.
    #[test]
    fn the_written_gpu_config_carries_the_capped_count_everywhere() {
        let gpu = gpu_presets()
            .into_iter()
            .find(|g| g.slug == "rx9070xt")
            .unwrap();
        ...
        let safe_assist = app::cpu_threads::cpu_assist_threads_for(logical);
        s.cpu = CpuPreset {
            label: "test: all cores".to_string(),
            supervene: all_cores,
        };
        let path =
            std::env::temp_dir().join(format!("hacash-panel-assist-{}.ini", std::process::id()));
        write_poworker_config(&path, &s).unwrap();
        let raw = std::fs::read_to_string(&path).unwrap();
        let _ = std::fs::remove_file(path);
        assert!(raw.contains(&format!("supervene = {safe_assist}")), "{raw}");
        assert!(
            raw.contains(&format!("supervene_max = {safe_assist}")),
            "{raw}"
        );
        assert!(raw.contains("cpu_assist = true"), "{raw}");
        if all_cores > safe_assist {
            assert!(
                !raw.contains(&format!("supervene = {all_cores}")),

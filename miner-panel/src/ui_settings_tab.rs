...
/// Why the panel will not start HBIT, in the operator's own language. The
/// decision is made in public_pool.rs and the wording lives in i18n.rs; this is
/// the only place the two meet.
fn hbit_refusal_text(t: &crate::i18n::Strings, r: crate::public_pool::HbitRefusal) -> &'static str {
    use crate::public_pool::HbitRefusal as R;
    match r {
        R::Node => t.hbit_err_node,
        R::WalletFile => t.hbit_err_wallet_file,
        R::Numbers => t.hbit_err_numbers,
        R::Chain => t.hbit_err_chain,
        R::PasswordMissing => t.hbit_err_pass_missing,
        R::PasswordShort => t.hbit_err_pass_short,
        R::PasswordMismatch => t.hbit_err_pass_mismatch,
        R::NotAcknowledged => t.hbit_err_not_acked,
        R::PortBusy => t.hbit_err_port_busy,
        R::AlreadyRunning => t.hbit_err_already_running,
    }
}

/// What the footer says about a start attempt, in one line, whichever way it
/// went. The section itself shows the same outcome in full.
fn hbit_outcome_text(
    t: &crate::i18n::Strings,
    connect: &str,
    outcome: Result<(), crate::public_pool::HbitStartError>,
) -> String {
    use crate::public_pool::HbitStartError as E;
    match outcome {
        Ok(()) => t.hbit_running_display(connect),
        Err(E::Refused(r)) => hbit_refusal_text(t, r).to_string(),
        Err(E::Missing(where_looked)) => format!("{} {where_looked}", t.hbit_missing),
        Err(E::Os(e)) => format!("{} {e}", t.hbit_start_failed),
    }
}
...
NAT/CGNAT often blocks it). This panel cannot verify external reachability - test from another network.",
            ...
            // Deliberately a second card, not another row in the one above. The
            // relay forwards work; this one owes miners money.
            ui.add_space(12.0);
            self.hbit_pool_section(ui);
        }
    ...
    /// HBIT, the pool that holds a wallet and pays other people.
    ///
    /// Its own card, its own words and its own start button, because the relay
    /// above forwards work while this one owes miners money. Nothing here is one
    /// click away from running: the passphrase is asked for on every start and
    /// never stored, and the backup warning has to be acknowledged for the
    /// wallet file in use before the pool can be started even once.
    fn hbit_pool_section(&mut self, ui: &mut egui::Ui) {
        use crate::public_pool;

        let t = self.t();
        let work_dir = self.work_dir.clone();
        let mut st = public_pool::hbit_state(&work_dir);
        ...
        theme::section_card().show(ui, |ui| {
            ui.label(
                egui::RichText::new(t.hbit_title)
                    .strong()
                    .size(12.0)
                    .color(theme::colors::GOLD),
            );
            ui.label(
                egui::RichText::new(t.hbit_what)
                    .size(11.5)
                    .color(theme::colors::TEXT_MUTED),
            );
            ui.add_space(6.0);
            theme::note(ui, theme::colors::GOLD, t.hbit_vs_relay);
            ...
            ui.add_space(8.0);
            // Nothing about a running pool can be edited: every one of these is
            // an argument it was started with.
            ui.add_enabled_ui(!running, |ui| {
                egui::Grid::new("hbit_pool_grid")
                    .num_columns(2)
                    .spacing([20.0, 10.0])
                    .show(ui, |ui| {
                        theme::field_label(ui, t.hbit_label_node);
                        if ui
                            .add(
                                egui::TextEdit::singleline(&mut st.settings.node)
                                    .desired_width(280.0)
                        ...
                        theme::field_label(ui, t.hbit_label_wallet_file);
                        ui.vertical(|ui| {
                            if ui
                                .add(
                                    egui::TextEdit::singleline(&mut st.settings.wallet_file)
                                        .desired_width(280.0)
                                        .hint_text("pool-wallet.key"),
                                )
                                .changed()
                            {
                                // A different wallet is a different thing to
                                // back up, so this session's tick stops counting
                                // for it.
                                st.ack_checked = false;
                                st.save();
                            }
                            ui.label(
                                egui::RichText::new(t.hbit_wallet_file_hint)
                                    .size(11.0)
                                    .color(theme::colors::TEXT_MUTED),
                            );
                        });
                        ...
                        theme::field_label(ui, t.hbit_label_port);
                        if ui
                            .add(egui::DragValue::new(&mut st.settings.port).range(1024..=65535))
                            .changed()
                        {
                            st.save();
                        }
                        ...
                        theme::field_label(ui, t.hbit_label_reach);
                        ui.horizontal(|ui| {
                            let public = st.settings.public_bind;
                            if ui.selectable_label(!public, t.hbit_reach_local).clicked() && public
                            {
                                st.settings.public_bind = false;
                                st.save();
                            }
                            if ui.selectable_label(public, t.hbit_reach_public).clicked() && !public
                            {
                                st.settings.public_bind = true;
                                st.save();
                            }
                        });
                        ui.end_row();

                        theme::field_label(ui, t.hbit_label_chain);
                        ui.vertical(|ui| {
                            let mainnet = st.settings.chain.trim() == "mainnet";
                            ui.horizontal(|ui| {
                                if ui.selectable_label(mainnet, t.hbit_chain_mainnet).clicked()
                                    && !mainnet
                                {
                                    st.settings.chain = "mainnet".into();
                                    st.save();
                                }
                                if ui
                                    .selectable_label(!mainnet, t.hbit_chain_testnet)
                                    .clicked()
                                    && mainnet
                                {
                                    st.settings.chain = "testnet".into();
                                    st.save();
                                }
                            });
                            // A testnet node takes its difficulty window and
                            // block time from its own config file, so let the
                            // exact pair be spelled out.
                            if !mainnet
                                && ui
                                    .add(
                                        egui::TextEdit::singleline(&mut st.settings.chain)
                                            .desired_width(200.0)
                                            .hint_text("testnet:288:10"),
                                    )
                                    .changed()
                            {
                                st.save();
                        ...
                        theme::field_label(ui, t.hbit_label_share_bits);
                        if ui
                            .add(egui::DragValue::new(&mut st.settings.share_bits).range(18..=40))
                            .changed()
                        {
                            st.save();
                        }
                        ...
                        theme::field_label(ui, t.hbit_label_settle);
                        if ui
                            .add(
                                egui::DragValue::new(&mut st.settings.settle_secs)
                                    .range(60..=86400),
                            )
                            .changed()
                        {
            ...
            // Said the moment the choice is made, not in a tooltip nobody opens.
            if st.settings.public_bind {
                ui.add_space(6.0);
                theme::note(ui, theme::colors::RED, t.hbit_public_now);
            }

            // A pool this panel did not start can still be running: the
            // operator's own from an earlier session, started here and left
            // running on purpose. Say so before they try to start a second.
            if !running && st.port_busy() {
                ui.add_space(6.0);
                theme::note(ui, theme::colors::GOLD, t.hbit_err_port_busy);
            }

            if !running {
                ui.add_space(10.0);
                let wallet_exists = st.wallet_exists();
                egui::Grid::new("hbit_pass_grid")
                    .num_columns(2)
                    .spacing([20.0, 10.0])
                    .show(ui, |ui| {
                        theme::field_label(ui, t.hbit_label_pass);
                        ui.add(
                        ...
                        ui.end_row();
                        // Only while the wallet is about to be CREATED. On an
                        // existing wallet a wrong passphrase simply fails to
                        // open it and costs nothing; on a new one it silently
                        // becomes the passphrase.
                        if !wallet_exists {
                            theme::field_label(ui, t.hbit_label_pass_again);
                            ui.add(
                                egui::TextEdit::singleline(&mut st.password_again)
                                    .password(true)
                                    .desired_width(280.0),
                            );
                            ui.end_row();
                ...
                ui.label(
                    egui::RichText::new(t.hbit_pass_hint)
                        .size(11.0)
                        .color(theme::colors::TEXT_MUTED),
                );
            }
            ...
            ui.add_space(10.0);
            let acknowledged = st.acknowledged();
            if running {
                if theme::btn_danger(ui, t.hbit_btn_stop).clicked() {
                    st.stop();
                }
                ui.add_space(6.0);
                ui.label(
                    egui::RichText::new(
                        t.hbit_running_display(&public_pool::hbit_local_connect(st.settings.port)),
                    )
                    .size(11.5)
                    .color(theme::colors::TEXT),
                );
                ui.add_space(6.0);
                theme::note(ui, theme::colors::GOLD, t.hbit_keeps_running);
            } else if acknowledged {
                // Already acknowledged for this wallet file. The warning stays
                // one click away rather than disappearing.
                egui::CollapsingHeader::new(t.hbit_backup_title)
                    .default_open(false)
                    .show(ui, |ui| {
                        ui.label(
                            egui::RichText::new(t.hbit_backup_body)
                                .size(11.5)
                                .color(theme::colors::TEXT_MUTED),
                        );
                    });
                ui.add_space(6.0);
                if theme::btn_primary(ui, t.hbit_btn_start).clicked() {
                    outcome = Some(st.start());
                }
            } else {
                // The gate. There is no start button on this path until the
                // person has read this and said so.
                theme::note(ui, theme::colors::RED, t.hbit_backup_body);
                ui.add_space(6.0);
                let mut ack = st.ack_checked;
                if ui.checkbox(&mut ack, t.hbit_backup_check).changed() {
                    st.ack_checked = ack;
                }
                ui.add_space(6.0);
                if ui
                    .add_enabled(st.ack_checked, egui::Button::new(t.hbit_backup_start))
                    .clicked()
                {
                    outcome = Some(st.start());
                }
            ...
            if let Some(r) = st.refusal {
                ui.add_space(6.0);
                ui.label(
                    egui::RichText::new(hbit_refusal_text(&t, r))
                        .size(11.5)
            ...
            if let Some(e) = &st.error {
                ui.label(
                    egui::RichText::new(format!("{} {e}", t.hbit_start_failed))
                        .size(11.5)
                        .color(theme::colors::RED),
                );
            }
            if let Some(e) = &st.save_error {
                ui.label(
                    egui::RichText::new(format!("{} {e}", t.hbit_save_failed))
                        .size(11.5)
                        .color(theme::colors::RED),
                );
            }
            // The pool's own last words, whatever they were. A refusal it
            // printed is the real reason it is not running, so it is shown
            // instead of being replaced by a guess of ours.
            if let Some(code) = &st.last_exit {
                ui.add_space(6.0);
                ui.label(
                    egui::RichText::new(t.hbit_exited_display(code))
                        .size(11.5)
                        .color(theme::colors::RED),
                );
            }
            ...
            ui.add_space(8.0);
            ui.label(
                egui::RichText::new(t.hbit_log_title)
                    .strong()
                    .size(11.5)
                    .color(theme::colors::TEXT_MUTED),
            );
            if st.log.is_empty() {
                ui.label(
                    egui::RichText::new(t.hbit_log_empty)
                        .size(11.0)
                        .color(theme::colors::TEXT_MUTED),
                );
            } else {
            ...
            ui.label(
                egui::RichText::new(format!("{} {}", t.hbit_log_where, st.log_path().display()))
                    .size(10.5)
                    .color(theme::colors::GOLD_DIM),
            );
        });

        // Say in the footer what the attempt did, the same way starting the
        // relay or the miner does.
        if let Some(outcome) = outcome {
            let connect = public_pool::hbit_local_connect(st.settings.port);
            drop(st);
            self.status_msg = hbit_outcome_text(&t, &connect, outcome);
        }
    }
